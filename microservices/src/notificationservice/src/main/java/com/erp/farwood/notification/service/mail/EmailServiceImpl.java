...
import com.erp.farwood.notification.dto.Attachment;
	...
	public void sendMail(SendMailDto mailDto) {
		try {
			...
			if (mailDto.isDisplayEmailSignature()) {
				...
				URL url = ResourceUtils.getURL("classpath:image/farwood-logo.png");
				File file = new File("farwood-logo.png");
				FileUtils.copyURLToFile(url, file);
				helper.addInline("farwood-logo", file);
			}
			...
			if (attachments != null && !attachments.isEmpty()) {
				for (Attachment attachment : attachments) {
					if (attachment.getFilePath() != null && !attachment.getFilePath().isEmpty()) {
						...
						logger.info("Documents to send in mail :" + file);
						...
						helper.addAttachment(attachment.getAttachmentName(),
								new ByteArrayDataSource(attachment.getFileByte(), attachment.getContentType()));
					}
